<template>
  <div>
    <div style="margin-bottom: 5px">
      ...
      <el-select v-model="customerId" placeholder="客户" style="margin-left: 5px; width: 200px" clearable>
        <el-option
      ...
      <el-table-column prop="operate" label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button 
            ...
            size="small" 
            type="info" 
            @click="complete(scope.row)">出库</el-button>
          <el-popconfirm 
    ...
    <el-dialog
      ...
      <el-form ref="form" :rules="rules" :model="form" label-width="80px">
        <el-form-item label="客户" prop="relatedPartyId">
          <el-select v-model="form.relatedPartyId" placeholder="请选择客户" style="width: 100%">
            <el-option
              v-for="item in customerOptions"
              :key="item.customerId"
              :label="item.customerName"
...
export default {
  ...
  computed: {
    ...mapState({
      tableData: state => state.sales.salesOrders,
      total: state => state.sales.total
    }),
    hasReviewPermission() {
  ...
  methods: {
    ...
    async loadPost() {
      ...
      const res = await this.getSalesOrders({
    ...
    async loadCustomerOptions() {
      try {
        const res = await this.getCustomers({ 
          pageNum: 1, 
          pageSize: 1000,
          params: { isValid: 1 } // 只获取有效的客户
        })
        if (res && res.code === 200) {
          this.customerOptions = res.data
          console.log('客户数据:', this.customerOptions) // 添加日志，查看加载的数据
        } else {
          this.$message.error('获取客户列表失败')
        }
      } catch (error) {
        console.error('加载客户选项失败', error)
        this.$message.error('加载客户选项失败: ' + error.message)
      }
    },
    ...
    async save() {
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          try {
            const res = await this.createSalesOrder(orderData)
            if (res && res.code === 200) {
    ...
    async submitStorage() {
      ...
      try {
        const res = await this.processSaleOut({
          orderId: this.currentOrder.orderId,
        ...
        if (res && res.code === 200) {
          ...
          if (res && res.msg && res.msg.includes('已经处理过')) {
            this.$message.warning('该订单已经完成出库操作')
            this.storageDialogVisible = false
            this.loadPost()
          } else {
            this.$message.error('出库失败: ' + (res ? res.msg : '未知错误'))
          }
        }
      } catch (error) {
